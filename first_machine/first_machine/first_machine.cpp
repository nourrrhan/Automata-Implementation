...
bool Is_Accepted_By_First_Machine(string test)
{
    string transition_table[3][3] = { " ", "state 0", "state 1",
                                      "0", "state 0", "state 0",
                                      "1", "state 1", "state 1" };
...
int main()
{
    ...
    string test;
    cout << "\tHello" << endl << "\tThis machine accepts the binary representation of odd numbers" << endl;
    cout << "\tPlease Enter a string to test or enter -1 to exit" << endl << "\t";
    cin >> test;
    cout << endl;
    while (test != "-1")
    {
        if (Is_Accepted_By_First_Machine(test))
            cout << "\t" << test << " is accepted by this machine because it is a binary representation of an odd number." << endl << endl;
        else
            cout << "\t" << test << " is rejected by this machine because it is not a binary representation of an odd number." << endl << endl;
